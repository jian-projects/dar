...
def random_parameters(args):
    ...
    if not args.train['seed_change']:
        ...
        print(f"lr_{lr}, lr_pre_{lr_pre}, bz_{bz}, dr_{dr}, seed_{seed}")
        return args
    ...
    lr, lr_pre = args.train['learning_rate'], args.train['learning_rate_pre']
    bz, dr, seed = args.train['batch_size'], args.model['drop_rate'], args.train['seed']
    print(f"change: lr_{lr}, lr_pre_{lr_pre}, bz_{bz}, dr_{dr}, seed_{seed}")
...
def get_metric(rec_items, dataset):
    labels, preds, losses = [], [], []
    if dataset.task == 'cls':
        for outs in rec_items:
            labels.extend(outs['labels'].cpu().numpy())
            preds.extend(outs['preds'].cpu().numpy())
            losses.append(outs['loss'].item())

        scores = {'loss': round(sum(losses)/len(losses), 2)}
        for metric in dataset.metrics:
            if 'acc' in metric:
                scores[metric] = round(accuracy_score(labels, preds), 4)
            if '_f1' in metric:
                scores[metric] = round(f1_score(labels, preds, labels=dataset.lab_range, average=metric.split('_')[0]), 4)
