...
class LViT(nn.Module):
    def __init__(self, config, n_channels=3, n_classes=1, img_size=224, vis=False):
        super().__init__()
        self.plm_model = AutoModel.from_pretrained(config.model['plm'], local_files_only=False) 
        for param in self.plm_model.parameters(): param.requires_grad = False
        self.vis = vis
        self.n_channels = n_channels
        self.n_classes = n_classes
        in_channels = 64 # config.base_channel
    ...
    def forward(self, inputs, stage='train'):
        x, lab, input_ids = inputs['p_img'], inputs['p_lab'], inputs['input_ids']
        text = self.plm_model(input_ids).last_hidden_state

        use_text = 0 # 用文本模态
        x = x.float()  # x [4,3,224,224]
        x1 = self.inc(x)  # x1 [4, 64, 224, 224]
        if use_text:text4 = self.text_module4(text.transpose(1, 2)).transpose(1, 2) 
        if use_text:text3 = self.text_module3(text4.transpose(1, 2)).transpose(1, 2)
        if use_text:text2 = self.text_module2(text3.transpose(1, 2)).transpose(1, 2)
        if use_text:text1 = self.text_module1(text2.transpose(1, 2)).transpose(1, 2)
        if use_text: y1 = self.downVit(x1, x1, text1)
        x2 = self.down1(x1)
        if use_text:y2 = self.downVit1(x2, y1, text2)
        x3 = self.down2(x2)
        if use_text:y3 = self.downVit2(x3, y2, text3)
        x4 = self.down3(x3)
        if use_text:y4 = self.downVit3(x4, y3, text4)
        x5 = self.down4(x4)
        if use_text:y4 = self.upVit3(y4, y4, text4, True)
        if use_text:y3 = self.upVit2(y3, y4, text3, True)
        if use_text:y2 = self.upVit1(y2, y3, text2, True)
        if use_text:y1 = self.upVit(y1, y2, text1, True)
        if use_text:x1 = self.reconstruct1(y1) + x1
        if use_text:x2 = self.reconstruct2(y2) + x2
        if use_text:x3 = self.reconstruct3(y3) + x3
        if use_text:x4 = self.reconstruct4(y4) + x4
        x = self.up4(x5, x4)
        x = self.up3(x, x3)
        x = self.up2(x, x2)
        x = self.up1(x, x1)
        ...
        loss = self.loss_bce(logits, lab.float())
        return {
            'logits': logits,
            'loss': loss,
            # 'preds': None,
